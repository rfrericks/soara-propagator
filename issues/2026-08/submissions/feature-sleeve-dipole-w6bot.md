...
- **Reprint permission — confirmed 2026-08-13.** Rich sent this article to Ray
  directly for inclusion in the Propagator, so permission to publish in full is
  granted notwithstanding the "© 2026, all rights reserved" mark.
- **Callsign — confirmed 2026-08-13.** W6BOT / Rich Gordon is correct; he's a
  former SOARA member who has since moved out of town, which is why he isn't in the
  current club roster.
...
### Editor actions taken for the August 2026 issue

- Moved from `holding/` into `issues/2026-08/`; frontmatter `issue:` changed from
  "July 2026" to "August 2026". Nine images moved to `issues/2026-08/assets/`.
- Article runs **in full** in `newsletter.md` per `CLAUDE.md` and
  `docs/editorial-style.md`. The earlier Editor Note above suggesting a condensed
  version predates that rule — flagged for the editor rather than acted on. No
  text, table, or figure was cut.
- Mechanical formatting only: headings demoted one level so the article title is
  the `##` section heading (the PDF builder's "In This Issue" list is built from
  `##`), and image paths changed from `../assets/` to `assets/` because
  `newsletter.md` sits at the issue root.
- The transcriber's line saying the remaining plots "are in `assets/` and can be
  included as space allows" referenced an internal folder, so all five remaining
  NanoVNA plots are now placed as real figures in the published section. Their
  captions (S11 log magnitude, Smith chart, impedance, phase, quality factor) are
  **editor-supplied descriptions of the author's plots**, not the author's words —
  review them, and confirm each label matches the correct plot.
- Editor Notes are not carried into `newsletter.md`; they stay in this file.
